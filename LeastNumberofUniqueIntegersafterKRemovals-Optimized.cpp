/*
Count numbers using a hash lookup. Then put the counts to a min heap. Remove elements from the min heap while k > 0.
...
int findLeastNumOfUniqueInts(vector<int>& arr, int k) {
    unordered_lookup<int, int> counts;
    vector<int> heap;
    for (auto n : arr)
        ++counts[n];
    for (auto &p: counts)
...
Memory: O(m) for the hash lookup and heap.
*/